...
# Create a very simple datagenerator including random brightness augmentation 
def datageneratorbright(images, segmentations, patch_size, patches_per_im, batch_size):
    """
    Simple data-generator to feed patches in batches to the network.
    To extract different patches each epoch, steps_per_epoch in fit_generator should be equal to nr_batches.
    ...
    # Here the brightness augmentation is inserted
    # This thus leads to a doubling of the training dataset
    
    for i in range(images.shape[0]):
        factor=0.5
        
        # Convert rgb to hsb to easily access the brightness value
        hsv = matplotlib.colors.rgb_to_hsv(images[i,:,:,:]) 
        hsv[:, :, 2] = hsv[:, :, 2] * (factor + np.random.uniform()) #scale channel V uniformly
        hsv[:, :, 2][hsv[:, :, 2] > 255] = 255 #reset out of range values
        im = np.array(matplotlib.colors.hsv_to_rgb(hsv))
        images = np.concatenate((images,im[None,:,:,:]), axis = 0);
...
# Create a very simple datagenerator including random brightness augmentation and spline transformation with gryds
def datageneratorbright_spline(images, segmentations, patch_size, patches_per_im, batch_size):
    """
    Simple data-generator to feed patches in batches to the network.
    To extract different patches each epoch, steps_per_epoch in fit_generator should be equal to nr_batches.
    ...
    # After the brightness augmentation we transform all images?
    # For now we only transform the images that also have varied brightness (so not the original part of the dataset)
    # Therefore range is from 20 to total image shape
    for i in range(20, images.shape[0]):
        ...
        transformed_image = np.zeros((images.shape[1], images.shape[2], images.shape[3]))
        for color in range(images.shape[3]):
            interpolator = gryds.Interpolator(images[i,:,:,color])
    
            # Transform the image using the B-spline transformation
            images[i,:,:,color] = interpolator.transform(bspline)
